...
all_dat <- movement_summary %>%
  ...
  mutate(log_intensity = log(intensity_use+.001)) %>%
...
# Model lm
glmmTMB::glmmTMB(log_speed ~ value +  (value | taxon), data = all_dat[all_dat$env_char == 'simpsons_D_sc',]) %>% 
  summary()

glmmTMB::glmmTMB(log_intensity ~ value +  (value | taxon), data = all_dat[all_dat$env_char == 'simpsons_D_sc',]) %>% 
  summary()

glmmTMB::glmmTMB(log_speed ~ value +  (value | taxon), data = all_dat[all_dat$env_char == 'pet_sc',]) %>% 
  summary()

glmmTMB::glmmTMB(log_intensity ~ value +  (value | taxon), data = all_dat[all_dat$env_char == 'pet_sc',]) %>% 
  summary()
...
# Effects on MCP area
ggplot(all_dat, aes(x = value, y = log_hr_area, colour = taxon)) + 
  geom_point() + 
  scale_colour_viridis_d() +
  facet_wrap(~env_char, scales = 'free_x') +
  theme(legend.position = 'none') +
...
ggplot(all_dat, aes(x = value, y = log_intensity)) + 
  geom_point() +
